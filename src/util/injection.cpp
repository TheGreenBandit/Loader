...
namespace loader
{
	...
	void injection::inject()
	{
		switch (g_gui.game)
		{
			case LETHAL_COMPANY:
			{
				...
				fs::path p = fs::current_path() / "Menus" / "LethalMenu";
				if (!std::filesystem::is_directory(p)) std::filesystem::create_directory(p);
				g_cpr_client.download("https://icyrelic.com/release/lethalmenu/LethalMenu.dll", (p / "lethalmenu.dll"));
				mono_inject();
			}break;
	...
	void injection::inject_dll(egame game, std::string_view npath, std::string_view nwindow)//egame is because when i make menu for phasmo it will also use this method
	{
		std::thread([=] 
			{
				fs::path path = npath;
	...
	void injection::mono_inject()
	{
		...
		HWND hwnd = FindWindowA(NULL, window);//todo stop using smi or figure out a way to make it not give alert everytime
