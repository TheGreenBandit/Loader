...
#define NOTIFY_TOAST_FLAGS \
	ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoSavedSettings
...
public:
	...
	NOTIFY_INLINE auto set_custom_icon(const char* icon) -> void
	{
		this->custom_icon = icon;
	}
...
public:
	...
	NOTIFY_INLINE auto get_color() -> const ImVec4&
	{
		switch (this->type)
		{
		case ImGuiToastType_None: return { 255, 255, 255, 140 };  // White
		case ImGuiToastType_Success: return { 0, 255, 0, 140 };   // Green
		case ImGuiToastType_Warning: return { 255, 255, 0, 140 }; // Yellow
		case ImGuiToastType_Error: return { 255, 0, 0, 140 };     // Error
		case ImGuiToastType_Info: return { 0, 157, 255, 140 };    // Blue
		default: return {};
		}
	}
	...
	NOTIFY_INLINE auto get_custom_icon() -> const char*
	{
...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			const auto custom_icon = current_toast->get_custom_icon();

			// Window rendering
			auto text_color = ImVec4(.228, .5, .8, 1);
			...
			// Here we render the toast content
			{
				...
				// If an icon is set
				if (custom_icon != "" || custom_icon != NULL)
				{
					TextColored(text_color, custom_icon);
